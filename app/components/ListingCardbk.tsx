...
export function ListingCard({ listing, isUserLoggedIn = true }: ListingCardProps) {
  ...
  return (
    <Link to={isUserLoggedIn ? `/listings/${listing.id}` : "/login"} className="card p-6 hover:shadow-md transition-shadow">
      {/* Header */}
  ...
  {isUserLoggedIn ? (
    <>
      ...
      <div className="text-right">
        {(listing.listing_type === "bib" || listing.listing_type === "room_and_bib") && listing.associated_costs ? (
          <>
            <p className="text-xs text-gray-500">Associated costs</p>
