...
export default function BibLanding() {
  ...
  const [preference, setPreference] = useState("bib");
          ...
          <div className="bib-form-card" id="request">
            {complete ? (
              <div className="bib-confirmation" ref={confirmation} tabIndex={-1}>
                ...
                <button className="bib-submit" type="button" onClick={() => { setDismissed(true); setRace(""); setOtherRace(""); }}>Request another race <span aria-hidden="true">↗</span></button>
              </div>
            ) : (
              <fetcher.Form method="post" onSubmit={() => setDismissed(false)}>
                ...
                <div className="bib-form-heading"><p className="bib-eyebrow">LET’S MAKE IT HAPPEN</p><h2>Where do you want to run?</h2><p>Choose a race. We’ll keep an eye out.</p></div>
                <fieldset className="bib-race-fieldset"><legend className="bib-sr-only">Choose your race</legend><div className="bib-race-grid">
                  {bibRaces.map((item) => <label className={`bib-race ${race === item.name ? "is-selected" : ""}`} key={item.name}>
                    <input type="radio" name="race" value={item.name} checked={race === item.name} onChange={() => setRace(item.name)} required />
                    <span className="bib-race-code" aria-hidden="true">{item.code}</span><span className="bib-race-name">{item.name}<small>{item.country}</small></span><span className="bib-radio-dot" aria-hidden="true" />
                  </label>)}
                </div></fieldset>
                {race === "Another race" && <label className="bib-field">Race name<input name="otherRace" value={otherRace} onChange={(event) => setOtherRace(event.target.value)} placeholder="e.g. Valencia Marathon" required maxLength={120} /></label>}
                <fieldset className="bib-preference"><legend>I’m looking for</legend><div>{[{ value: "bib", text: "Bib only" }, { value: "package", text: "Full package (bib + hotel)" }].map(item => <label key={item.value}><input type="radio" name="preference" checked={preference === item.value} onChange={() => setPreference(item.value)} value={item.value} /><span>{item.text}</span></label>)}</div></fieldset>
                ...
                {!dismissed && result?.error && <p role="alert" className="bib-error">{result.error}</p>}
                <button className="bib-submit" type="submit" disabled={submitting}>{submitting ? "Saving your request…" : race && race !== "Another race" ? `Notify me about ${race}` : "Notify me about my race"}<span aria-hidden="true">↗</span></button>
