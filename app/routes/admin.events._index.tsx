...
export const meta: MetaFunction = () => [{ title: "Events - Admin - Runoot" }];
...
export default function AdminEventRequestsPage() {
  ...
  const initialTab = (["online", "requests", "offline"] as const).includes(searchParams.get("tab") as any)
    ? (searchParams.get("tab") as "online" | "requests" | "offline")
    : "requests";
  const [activeTab, setActiveTab] = useState<"online" | "requests" | "offline">(initialTab);
  const [previewImage, setPreviewImage] = useState<{ url: string; alt: string } | null>(null);

  const todayStr = useMemo(() => new Date().toISOString().slice(0, 10), []);
  const allRequests = useMemo(() => requests || [], [requests]);

  const isOnline = (req: any) =>
    req.status === "published" && !req.archived_at && req.event_date >= todayStr;
  const isOffline = (req: any) =>
    !!req.archived_at || (req.status === "published" && req.event_date < todayStr);
  const isRequest = (req: any) =>
    req.status !== "published" && !req.archived_at;

  const onlineCount = useMemo(() => allRequests.filter(isOnline).length, [allRequests, todayStr]);
  const requestsCount = useMemo(() => allRequests.filter(isRequest).length, [allRequests, todayStr]);
  const offlineCount = useMemo(() => allRequests.filter(isOffline).length, [allRequests, todayStr]);

  const visibleRequests = useMemo(() => {
    const filterFn = activeTab === "online" ? isOnline : activeTab === "offline" ? isOffline : isRequest;
    return allRequests.filter(filterFn).sort((a: any, b: any) => {
  ...
  }, [activeTab, allRequests, todayStr]);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-gray-900">Events</h1>
          <p className="text-gray-500 mt-1">Manage event requests, published events and archive.</p>
        </div>
        <Link
          to="/admin/events/new"
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        {(["online", "requests", "offline"] as const).map((tab) => {
          const count = tab === "online" ? onlineCount : tab === "requests" ? requestsCount : offlineCount;
          const label = tab === "online" ? "Online" : tab === "requests" ? "Requests" : "Offline";
          return (
            <button
              key={tab}
              type="button"
        ...
        <div className="space-y-3 p-3">
          {visibleRequests.length > 0 ? (
                  ...
                  <div className="flex items-center gap-2">
                    {activeTab === "online" && (
                      <Form method="post" className="inline">
                        ...
                        <button
                          type="submit"
                          onClick={(e) => { if (!confirm(`Move "${req.event_name}" to Offline?`)) e.preventDefault(); }}
                          className="h-10 px-4 rounded-full border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 text-sm font-semibold"
                        >
                          Move to Offline
                        </button>
                      </Form>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/95 p-6 text-sm text-gray-500">
              {activeTab === "online" ? "No published events online." : activeTab === "offline" ? "No offline events." : "No event requests."}
            </div>
          )}
        </div>
      </div>
